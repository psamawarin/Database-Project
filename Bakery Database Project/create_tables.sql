...
-- FIX: added NOT NULL on order_id/item_type/quantity/unit_price,
-- and a CHECK constraint restricting item_type to the two valid values.
CREATE TABLE ORDER_ITEM (
            order_item_id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
            order_id INT NOT NULL,
            item_type VARCHAR(50) NOT NULL CHECK (item_type IN ('product', 'custom_cake')),
            quantity INT NOT NULL,
            unit_price DECIMAL(10, 2) NOT NULL,
            FOREIGN KEY (order_id) REFERENCES CUSTOMER_ORDER (order_id)
);
...
CREATE TABLE ORDER_ITEM_CUSTOM_CAKE (
            ...
            PRIMARY KEY (order_item_id, custom_cake_id),
            FOREIGN KEY (order_item_id) REFERENCES ORDER_ITEM(order_item_id)
);
...
-- ============================================================
-- INDEXES (added for common FK lookups)
-- ============================================================
...
CREATE INDEX idx_pickup_slot_branch ON PICKUP_SLOT(branch_id);
CREATE INDEX idx_product_inventory_branch ON PRODUCT_INVENTORY(branch_id);
CREATE INDEX idx_ingredient_inventory_branch ON INGREDIENT_INVENTORY(branch_id);
...
CREATE INDEX idx_customer_order_pickup_slot ON CUSTOMER_ORDER(pickup_slot_id);
CREATE INDEX idx_payment_order ON PAYMENT(order_id);
CREATE INDEX idx_delivery_order ON DELIVERY(order_id);
CREATE INDEX idx_delivery_address ON DELIVERY(address_id);
CREATE INDEX idx_order_item_order ON ORDER_ITEM(order_id);
CREATE INDEX idx_order_item_product_product ON ORDER_ITEM_PRODUCT(product_id);
CREATE INDEX idx_order_item_custom_cake_cake ON ORDER_ITEM_CUSTOM_CAKE(custom_cake_id);
CREATE INDEX idx_custom_cake_size ON CUSTOM_CAKE(size_id);
CREATE INDEX idx_custom_cake_shape ON CUSTOM_CAKE(cake_shape_id);
CREATE INDEX idx_custom_cake_layer_frosting ON CUSTOM_CAKE_LAYER(frosting_id);
CREATE INDEX idx_custom_cake_layer_base ON CUSTOM_CAKE_LAYER(base_id);